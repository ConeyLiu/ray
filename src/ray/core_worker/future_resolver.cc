...
void FutureResolver::ResolveFutureAsync(const ObjectID &object_id,
  ...
  auto callback = [this, object_id](const Status &status, const rpc::GetObjectStatusReply &reply) {
      if (!status.ok()) {
      ...
      // plasma.
      RAY_CHECK_OK(in_memory_store_->Put(RayObject(rpc::ErrorType::OBJECT_IN_PLASMA), object_id));
  };
  RAY_CHECK_OK(it->second->GetObjectStatus(request, callback));
}
